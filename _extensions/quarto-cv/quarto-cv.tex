...
% Make lists without bullets  # TURN OFF FOR CV 
...
$if(title-meta)$
            pdftitle={$if(author-meta)$$author-meta$: $endif$ $title-meta$ (L Price)},
$endif$
$if(author-meta)$
...
$if(email)$$if(fontawesome)$\faEnvelope \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:lauren.3.price@gmail.com}{\tt $email$} \hspace{1 mm}$endif$ 
$if(phone)$$if(fontawesome)$ \faPhone \hspace{1 mm}$else$\emph{Phone:}$endif$  $phone$  \hspace{1 mm} $endif$ 
$if(github)$$if(fontawesome)$\faGithub \hspace{1 mm}$else$\emph{Github:}$endif$ \href{http://github.com/pricele2}{\tt $github$} \hspace{1 mm} $endif$  
...
% $if(web)$$if(fontawesome)$\faGlobe \hspace{1 mm}$else$\emph{Web:}$endif$ \href{https://www.linkedin.com/in/lauren-e-price-mpp/}{\tt $web$}  $endif$ 
